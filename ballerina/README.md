...
[DocuSign](https://www.docusign.com/) is a cloud-based electronic signature and agreement management platform that enables organizations to securely send, sign, and manage documents digitally, streamlining workflows and accelerating business transactions.
...
2. Note that the DocuSign Admin API requires an Organization-level account with Admin privileges. API access to Admin features is available on DocuSign Business Pro, Enterprise Pro, or higher plans.
...
2. Click **Add App and Integration Key** to create a new application and generate an Integration Key (Client ID).

3. Under your application settings, add a Secret Key by clicking **Add Secret Key**. Copy and securely store this value.

4. Configure your authentication method (Authorization Code Grant or JWT) and add the required redirect URIs for your application.

5. To obtain an access token, complete the OAuth 2.0 authentication flow using your Integration Key and Secret Key through the DocuSign OAuth endpoints.
