...
2. Note that the DocuSign Admin API requires an Organization-level account with Admin privileges. Access to the Admin API is available on DocuSign Enterprise Pro plans or higher, and your account must be part of a DocuSign Organization.
...
3. Under **Apps and Keys**, click on **Add App and Integration Key** to create a new integration.

4. Provide an App Name and click **Create App**.

5. Copy the **Integration Key** (Client ID) that is generated for your application.

6. Under **Authentication**, add a **Redirect URI** and configure your preferred authentication method (Authorization Code Grant or JWT).

7. For quick testing, you can generate an access token directly from the Developer Center by navigating to **Apps and Keys**, selecting your app, and clicking **Generate Access Token** under the Actions menu.
