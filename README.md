...
[DocuSign](https://www.docusign.com/) is a leading electronic signature and agreement cloud platform that enables organizations to automate and connect entire agreement processes, from preparing and signing to acting on and managing contracts and agreements.
...
2. Ensure you have a DocuSign Organization with Admin access, as the DocuSign Admin API requires organization-level permissions and is available to accounts with DocuSign Admin enabled (typically Enterprise plans or accounts with Organization Management features).

### Step 2: Generate an API Access Token

1. Log in to your DocuSign developer account at the [DocuSign Developer Portal](https://developers.docusign.com/).

2. Navigate to **My Apps & Keys** from the Apps and Keys section in your developer dashboard.

3. Click **Add App and Integration Key** to create a new application, or select an existing application.

4. Copy your **Integration Key** (Client ID) and configure your authentication settings (either Authorization Code Grant or JWT Grant).

5. For quick testing, you can generate an access token by navigating to the **Get Access Token** section in your app settings or using the [OAuth Token Generator](https://developers.docusign.com/tools/oauth-token-generator/) tool in the developer portal.

6. Ensure you have claimed your organization and enabled Admin API access by navigating to **Settings > Organization Management** in your DocuSign eSignature account.
