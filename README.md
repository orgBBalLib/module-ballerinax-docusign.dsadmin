...
[DocuSign](https://www.docusign.com/) is a leading electronic signature and agreement cloud platform that enables organizations to automate and connect entire agreement processes, from preparation to signing, acting, and managing contracts and documents digitally.
...
2. Ensure you have a DocuSign Organization with Admin capabilities enabled. The DocuSign Admin API requires an Enterprise Pro plan or higher with Organization Administration features activated.

### Step 2: Generate an API Access Token

1. Log in to the [DocuSign Developer Portal](https://developers.docusign.com/) and access your developer account.

2. Navigate to **My Apps & Keys** from the top navigation menu or your account settings.

3. Click **Add App and Integration Key** to create a new application, or select an existing application.

4. Copy your **Integration Key** (Client ID) which will be used for authentication.

5. Under the **Authentication** section, configure your authentication method (Authorization Code Grant or JWT Grant) and add the required redirect URIs.

6. For JWT authentication, generate an **RSA Key Pair** by clicking **Generate RSA** and securely save both the public and private keys.

7. To obtain an access token, use the DocuSign OAuth flow with your Integration Key and authentication credentials.
